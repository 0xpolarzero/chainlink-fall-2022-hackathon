...
export class ChildContractCreated extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ChildContractCreated entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ChildContractCreated must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ChildContractCreated", id.toString(), this);
    }
  }

  static load(id: string): ChildContractCreated | null {
    return changetype<ChildContractCreated | null>(
      store.get("ChildContractCreated", id)
    );
  }
